...
export default function TailorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowResults(true);
      setMessage('Resume tailored successfully! Here are your AI-optimized suggestions.');
      setIsSuccess(true);
           ...
           <div className="mt-8 bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
             ...
             <div className="space-y-6">
               ...
               {suggestions.experienceRewrites && suggestions.experienceRewrites.length > 0 && (
                 <div className="p-6 bg-orange-50 rounded-xl border border-orange-200">
                   ...
                   <div className="space-y-4">
                     {suggestions.experienceRewrites.map((rewrite: any, index: number) => (
                       <div key={index} className="border-l-4 border-orange-300 pl-4">
                         <h4 className="font-medium text-orange-900 mb-2">{rewrite.title || `Experience ${index + 1}`}</h4>
                         <p className="text-orange-800 text-sm">{rewrite.suggestion}</p>
                       </div>
                     ))}
                   </div>
                 </div>
               ...
               <div className="flex gap-4 pt-6 border-t border-gray-200">
                 <button className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 px-6 rounded-xl font-semibold hover:from-blue-700 hover:to-purple-700 transition-all duration-200">
                   Download Tailored Resume
                 </button>
                 <button className="flex-1 border-2 border-gray-300 text-gray-700 py-3 px-6 rounded-xl font-semibold hover:border-gray-400 hover:text-gray-900 transition-all duration-200">
                   Save to My Resumes
                 </button>
               </div>
             </div>
